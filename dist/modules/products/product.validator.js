...
exports.productSchema = joi_1.default.object({
    ...
    category: joi_1.default.string().valid('Electronics', 'Clothing', 'Home', 'Beauty', 'Toys', 'Sports', 'Automotive', 'Books').required()
        .messages({
        'string.base': 'Category should be a type of text',
        'any.only': 'Category must be one of {#valids}',
        'any.required': 'Category is a required field'
    }),
    tags: joi_1.default.array().items(joi_1.default.string().min(1).max(30)).min(1).required()
        .messages({
    ...
    variants: joi_1.default.array().items(joi_1.default.object({
        type: joi_1.default.string().valid('Color', 'Size', 'Storage Capacity', 'Style').required()
            .messages({
        ...
        value: joi_1.default.string().min(1).max(50).required()
            .messages({
